...
class ProgramOutput extends React.Component {
  render() {
    var items = Program.display(this.props.baseES, this.props.percentES, this.props.flatES, this.props.percentBlock, this.props.iLvl).map(function(item, index) {
          return (
            <TableRow key={index}>
              ...
              <TableRowColumn>{item.minES}</TableRowColumn>
              <TableRowColumn>{item.maxES}</TableRowColumn>
            </TableRow>
          );
        });
    return (
      <Table>
        <TableHeader displaySelectAll={false} adjustForCheckbox={false}>
          <TableRow>
            ...
            <TableHeaderColumn>Min ES</TableHeaderColumn>
            <TableHeaderColumn>Max ES</TableHeaderColumn>
          </TableRow>
        </TableHeader>
    ...
    return (
      <div>
        ...
        <div className="col-md-6">
          <ProgramOutput baseES={this.state.baseES} percentES={this.state.percentES} flatES={this.state.flatES} percentBlock={this.state.percentBlock} iLvl={this.state.iLvl} />
        </div>
      </div>
    );
  };
